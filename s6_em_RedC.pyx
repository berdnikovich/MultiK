...
#без изменений
@cython.profile(False)# эта штука профилирует время для каждого этапа работы
def read_spline_prior(str splineFilePath):
    ...
    cdef str line
    cdef list splineLine# задаем переменные для цикла далее
...
#новая функция, адаптированная под - значения расстояний    
@cython.profile(False)
cdef double get_spline_probability(dict spline, double interProb, str pairString):    
    """
    init pi based on chr and pos distance
    """
    cdef long pos1, pos2, dist, splineLen2, splineLen1, xPoint1, xPoint2 
    cdef str chr1, chr2
    cdef list pair, splineList
    cdef int indz
    ...
    if ch1 != ch2:
        ...
        if dist < 0:#upstr
            ...
            xPoint2 = min(bisect.bisect_right(splineList[indz+1:], dist), 0)
