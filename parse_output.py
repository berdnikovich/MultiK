...
uni_reads = os.path.join(outputPath, 's4/UNI_contacts_ALL_500000.Red_c')

##visualisation/tables
global map_prior_pic
map_prior_pic = os.path.join(outputPath, "s6/prior.pdf")
global heatmap_multi_prob
heatmap_multi_prob = os.path.join(outputPath, 's6/heatmap_multi_prob.pdf')
global heatmap_multi_post_max
heatmap_multi_post_max = os.path.join(outputPath, "s6/heatmap_multi_post_max.pdf")
global heatmap_multi_post_max_uni
heatmap_multi_post_max_uni = os.path.join(outputPath, "s6/heatmap_multi_post_max_uni.pdf")
...
sns.heatmap(map_multi_post)
plt.title("Multi reads with max probabilities", fontsize =20)
plt.xlabel("DNA-part (bin index)")
plt.ylabel("RNA-part (bin index)")
plt.savefig(heatmap_multi_post_max, bbox_inches = 'tight')
...
df_multi["true_pair"] = df_multi.apply(lambda row: 1 if row.id.split('_')[:2] == [str(int((row.st+250000)/500000)), str(int((row.end+250000)/500000))] else 0, axis=1)
...
result_prob = (len(df_model_true)/df_multi["true_pair"].value_counts()[1])*100
