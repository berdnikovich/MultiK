...
df_uni_rna_dna = pd.DataFrame({'ch1': li_ch, 'st' : df_rna_dna1['st']*500000-250000,# rna='start'
                                        'ch2': li_ch ,'end' : df_rna_dna1['end']*500000-250000, #dna='end'
...
sns.heatmap(df_uni)
plt.title("UNIreads bin is {}kbp".format(BL//1000), fontsize =20)
plt.xlabel("DNA-part (bin index)")
plt.ylabel("RNA-part (bin index)")
plt.savefig(unipic, bbox_inches = 'tight')
...
with open(logfile, 'a') as log:
        ...
        log.write("\n\nRandom probabitity for all multi parts = {}\n".format(count_true/count_true_false*100))
...
df_rna_dna_m = pd.DataFrame({'id': li_id_multi, 'ch1': li_ch, 'st' : [i*500000-250000 for i in li_rna_multi], 'ch2': li_ch,'end' : [i*500000-250000 for i in li_dna_multi]})
...
sns.heatmap(map_multi_df)
plt.title("Multireads bin is {}kbp".format(BL//1000), fontsize =20)
plt.xlabel("DNA-part (bin index)")
plt.ylabel("RNA-part (bin index)")
plt.savefig(multipic, bbox_inches = 'tight')
...
df_rna_dna_m_true = pd.DataFrame({'id': li_id_multi_true, 'ch1': li_ch, 'st' : [i*500000-250000 for i in li_rna_multi_true], 'ch2': li_ch,'end' : [i*500000-250000 for i in li_dna_multi_true]})
df_rna_dna_m_true['count'] = 1

map_multi_true_df = build_map(CL, BL, df_rna_dna_m_true)
sns.heatmap(map_multi_true_df)
plt.title("True multireads bin is {}kbp".format(BL//1000), fontsize =20)
plt.xlabel("DNA-part (bin index)")
plt.ylabel("RNA-part (bin index)")
plt.savefig("true_multi_map.pdf", bbox_inches = 'tight')
...
df_rna_dna_m_false = pd.DataFrame({'id': li_id_multi_false, 'ch1': li_ch, 'st' : [i*500000-250000 for i in li_rna_multi_false], 'ch2': li_ch,'end' : [i*500000-250000 for i in li_dna_multi_false]})
df_rna_dna_m_false['count'] = 1

map_multi_false_df = build_map(CL, BL, df_rna_dna_m_false)

sns.heatmap(map_multi_false_df)
plt.title("False multireads bin is {}kbp".format(BL//1000), fontsize =20)
plt.xlabel("DNA-part (bin index)")
plt.ylabel("RNA-part (bin index)")
plt.savefig("false_multi_map.pdf", bbox_inches = 'tight')